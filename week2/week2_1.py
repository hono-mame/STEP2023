...
def calculate_hash(key): 
    assert type(key) == str # Returns an error if key is not a string
    # Note: This is not a good hash function. Do you see why?
    # Answer: Because the hash values are very likely to collide.
    hash = 0
    for i in key:
        hash += ord(i)
    return hash

# Weighting according to the location of the characters in the string.
############################################################################
# Example:
# key1 = 'abc'
# hash =  5^2 * ord('a') + 5^1 * ord('b') + 5^0 * ord('c') = 3014
# key2 = 'cba'
# hash =  5^2 * ord('c') + 5^1 * ord('b') + 5^0 * ord('a') = 3062
...
def calculate_hash2(key):
    ...
    value = (len(key) - 1)
    for letter in key:
        hash += 3 ** value * ord(letter)
        value -= 1
    return hash

def calculate_hash3(key):
    assert type(key) == str
    hash = 0
    for letter in key:
        hash = hash * 127 + ord(letter) # Maximum value of ASCII code is 127
    return hash
...
class HashTable:
    ...
    def put(self, key, value):
        ...
        self.calculate_and_change_size() 
        bucket_index = calculate_hash2(key) % self.bucket_size
        item = self.buckets[bucket_index]
        while item: # Uses a loop to traverse the linked list, 
                    #looking for items with matching keys. If the key is found,
                    # it updates the value and returns False.
            if item.key == key:
                item.value = value
                return False
            item = item.next
    ...
    def delete(self, key):
        ...
        while item: 
            if item.key == key: # if the item is found
                if prev_item != None : # if the previous item exists
                    prev_item.next = item.next  # change .next
                else:
                    self.buckets[bucket_index] = item.next  #Update bucket reference to the next 
    ...
    def calculate_and_change_size(self):
    ...
    def rehash(self, new_bucket_size):
        ...
        for item in self.buckets:
            while item:
                new_bucket_index = calculate_hash2(item.key) % new_bucket_size
